...
def train_classifier(args,net, train_loader, val_loader, test_loader,
              epochs: int = 100,
              batch_size: int = 16,
    ...
    for epoch in range(1, epochs+1):
        ...
        ### Evaluation round
        val_acc, auroc, f1score, class_report_pdf = evaluate(net,val_loader, args.device, args)
        scheduler.step()

        logging.info('Validation Accuracy: {}'.format(val_acc))
        logging.info('Validation auroc: {}'.format(auroc))
    ...
    test_acc, test_auroc, test_f1score, test_class_report_pdf = evaluate(net,test_loader, args.device, args)
    wandb_log.log({
            'learning rate': optimizer.param_groups[0]['lr'],
...
def evaluate(net, dataloader, device,args):
    ...
    for image, labels,_ in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
        ...
        with torch.no_grad():
            ...
            all_targets.extend((labels.detach().cpu().numpy()).astype(np.int64))
            all_predictions.extend((outputs.detach().cpu().numpy()).astype(np.int64))   
    ...
    # compute mean AUROC
    auroc = utils.computeAUROC(all_targets,all_predictions, args.num_classes)

    # compute F1score
    f1score = utils.get_f1_score(all_targets,all_predictions)

    # classification report
    class_report_pdf = utils.class_report(all_targets,all_predictions)

    net.train()
    return acc, auroc, f1score, class_report_pdf
